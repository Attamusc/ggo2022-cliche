class Camera extends GameChildProcess {
	...
	inline function get_cWid()
		return M.ceil(pxWid / Const.GRID);

	public var cHei(get, never): Int;

	inline function get_cHei()
		return M.ceil(pxHei / Const.GRID);
	...
	**/
	public inline function isOnScreenRect(x: Float, y: Float, wid: Float, hei: Float, padding = 0.) {
		return Lib.rectangleOverlaps(pxLeft - padding, pxTop - padding, pxWid + padding * 2, pxHei + padding * 2, x, y, wid, hei);
	}
	...
	**/
	public inline function isOnScreenCase(cx: Int, cy: Int, padding = 32) {
		return cx * Const.GRID >= pxLeft - padding
			&& (cx + 1) * Const.GRID <= pxRight + padding
				&& cy * Const.GRID >= pxTop - padding
				&& (cy + 1) * Const.GRID <= pxBottom + padding;
	}
	...
	**/
	...
	public inline function levelToGlobalX(v: Float)
		return v * Const.SCALE + Game.ME.scroller.x;

	public inline function levelToGlobalY(v: Float)
