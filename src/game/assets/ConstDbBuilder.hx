...
class ConstDbBuilder {
				...
				expr: macro {
					...
					for (k in Reflect.fields(json))
						if (Reflect.hasField(this, k)) {
							try
								Reflect.setField(this, k, Reflect.field(json, k))
							catch (_)
								trace("ERROR: couldn't update JSON const " + k);
						}
	...
	**/
					...
					for (v in l.values) {
						...
						if (v.subValues != null && Reflect.fields(v.subValues).length > 0) {
							...
							for (k in Reflect.fields(v.subValues)) {
								...
								var const: Constant = !subValueTypes.exists(k)? CFloat(Std.string(rawVal)): switch subValueTypes.get(k).typeStr {
	...
	static function resolveFilePath(basePath: String): Null<String> {
		...
		if (path == null) {
			...
			if (r != null) {
				...
				if (!StringTools.endsWith(r, "/"))
					r += "/";
				try
					path = Context.resolvePath(r + basePath)
				catch (e: Dynamic)
					null;
			}
		}
		...
		if (path == null)
			try
				path = Context.resolvePath("res/" + basePath)
			catch (e: Dynamic)
				null;
